...
class TeamSelection extends Component {
  ...
  componentDidMount() {
    ...
    var allteams = this.props.fetchAllClubs("all", []).payload;
    console.log(allteams);

    localStorage.setItem("clubs", JSON.stringify(allteams));
  ...
  renderClubSelect(club, index) {
    return (
      <div
        className={local.searchContainer_listitem}
        onClick={() => this.singleClubClicked(club, index)}
        key={index}
      >
        ...
        <img
          ...
          style={
            club.selected ? { visibility: "visible" } : { visibility: "hidden" }
          }
        />
      </div>
    );
  ...
  renderSelectedClubs(club) {
    return (
      <Col
        ...
        lg={4}
        md={4}
        className={local.selectedContainer_clublist_main}
        key={club.name}
      >
        <div className={local.selectedContainer_clublist_item}>
          <p className={local.item_name}>
            {club.club.length > 12
            ...
            <div>
              ...
              <div className={local.searchContainer_clublist}>
                {this.props.clubs.map((club, index) =>
                  this.renderClubSelect(club, index)
                )}
              </div>
            </div>
          </Col>
                ...
                {this.state.selectedClubs.length === 0 ? (
                  ...
                  >
                    <div className={local.selectedContainer_clublist_item}>
                      ...
                      />
                      <p className={local.item_name}>Chosen Teams</p>
                    </div>
                  </Col>
                ) : (
                  this.state.selectedClubs.map((club) =>
                    this.renderSelectedClubs(club)
                  )
                )}
              </Row>
